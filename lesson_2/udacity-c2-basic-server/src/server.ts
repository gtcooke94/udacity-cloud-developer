...
(async () => {
  ...
  app.get("/cars",
    async(req: Request, res: Response) => {
      // Magic, will automatically make cars into json for response!
      let {make} = req.query
  ...
  app.post("/cars/",
    async(req: Request, res: Response) => {
      ...
      // Via testing, the order in the body doesn't matter, this works if the variable names match the body keys
      let {make, type, model, cost, id} = req.body

      if (!make || !type || !model || !cost || !id) {
        return res.status(400).send("Requires make, type, model, cost and id")
      }
